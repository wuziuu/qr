...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $targetR = 0x39; $targetG = 0x94; $targetB = 0xC3;
...
<head>
    ...
    <title>Bank QR Generator (Blue Version)</title>
    <style>
        body { font-family: sans-serif; background: #f0f7ff; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; }
        .card { background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); text-align: center; width: 100%; max-width: 400px; border-top: 5px solid #3994C3; }
        h2 { color: #3994C3; margin-bottom: 1.5rem; }
        .upload-area { border: 2px dashed #3994C3; padding: 2rem; border-radius: 8px; cursor: pointer; transition: 0.3s; margin-bottom: 1rem; color: #3994C3; background: #fdfdfd; }
        .upload-area:hover { background: #f0f9ff; }
        #preview { max-width: 100%; margin-top: 1rem; border-radius: 8px; display: none; box-shadow: 0 4px 10px rgba(0,0,0,0.1); }
        .btn { background: #3994C3; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; width: 100%; font-weight: bold; font-size: 1rem; }
        .btn:hover { background: #2b7da8; }
        .btn:disabled { background: #ccc; }
        #loading { display: none; margin: 10px 0; font-weight: bold; color: #3994C3; }
    </style>
</head>
<body>

<div class="card">
    <h2>Blue QR Generator</h2>
    <p style="color: #666;">Upload QR to apply blue theme and perspective warping.</p>
    
    <div class="upload-area" id="drop-zone" onclick="document.getElementById('fileInp').click()">
        <span>Click or Drag QR code here</span>
        <input type="file" id="fileInp" accept="image/*" hidden>
    </div>

    <div id="loading">Processing Perspective & Color...</div>
    <img id="preview" src="" alt="Result Preview">
            ...
            try {
                ...
                if (data.success) {
                    ...
                    dlBtn.onclick = () => {
                        ...
                        link.download = 'bank_transfer_blue.jpg';
                        link.click();
                    };
                } else {
